...
export async function POST(req) {
  ...
  try {
    ...
    if (!title || !Number.isFinite(Number(bpm)) || Number(bpm) < 40 || Number(bpm) > 240 || !key || !previewUrl) return NextResponse.json({ error: "タイトル・BPM・Key・試聴用ファイルを確認してください" }, { status: 400 });
    ...
    if (!finalThumbnailUrl) {
      ...
      const blob = await put(`thumbnails/${finalId}-${nanoid(8)}.svg`, new Blob([svg], { type: "image/svg+xml" }), { access: "public", addRandomSuffix: true, contentType: "image/svg+xml" });
      finalThumbnailUrl = blob.url;
    }
    stage = "database save";
    const beat = { id: finalId, title: String(title).trim(), genre: String(genre || ""), bpm: Number(bpm), key: String(key).trim(), producer: process.env.SELLER_LEGAL_NAME || "BEAT STORE", coverColor: coverColor || "#789ca0", thumbnailUrl: finalThumbnailUrl, previewFile: previewUrl, files: Object.fromEntries(Object.entries(files).filter(([, url]) => Boolean(url))), prices: normalizedPrices, visibility, createdAt: new Date().toISOString() };
    await saveBeat(beat);
    if (notifySubscribers && visibility !== "draft") {
      try { await sendBeatAnnouncement({ beat, earlyAccess: visibility === "early_access" }); } catch (emailError) { console.error("beat announcement error:", emailError); }
    }
